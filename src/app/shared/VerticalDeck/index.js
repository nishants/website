...
const itemInitialStyle = {position: 'absolute', top: 0, left: 0, opacity: 0, pointerEvents: "none"};

const computeLayout = ({container, items, sortBy, itemBottomMargin = 10,  itemRightMargin = 10}) => {
  const positions = [];
  const rowWidth = 270;
  const totalRows = Math.floor(container.offsetWidth / (rowWidth + itemRightMargin));

  let row = 0, lastYPositions = [];
  const sortedItems = items.sort(sortBy);

  for(let i =0; i < totalRows; i++) lastYPositions.push(0);

  for(let i =0; i < sortedItems.length; i++){
    const left = row * rowWidth + (row === 0 ? 0 : itemRightMargin);
    const top  = lastYPositions[row];
    const item = sortedItems[i];

    positions.push({item, left, top});

    lastYPositions[row] = (lastYPositions[row] + item.getHeight() + itemBottomMargin);
...
class VerticalDeck extends React.PureComponent{
  ...
  componentDidMount(){
    ...
    this.listener = this.gridElement.current.addEventListener("DOMSubtreeModified", this.updateLayout);
  }
  ...
  addItemRef = (item) => {
    ...
    this.setState(state => ({...state, gridItems: state.gridItems.concat(gridItem)}))
  };
  ...
  render(){
    ...
    return (
      <ul ref={gridElement} className='search-tab-items' style={{position: 'relative'}}>
        {items.map(Item => <li style={itemInitialStyle} id={`${gridId}-${Item.id}`} ref={addItemRef} key={Item.id}>{Item.Component}</li>)}
      </ul>
    );
  };
}
